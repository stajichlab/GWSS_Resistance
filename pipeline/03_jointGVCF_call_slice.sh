#!/usr/bin/bash
#SBATCH --mem 24G --nodes 1 --ntasks 2 --out logs/GVCFGenoGATK4.slice%a.log  -p intel,batch
...
if [ ! -f $GENOMEFOLDER/$REFGENOME ]; then
    module load samtools/1.11
    samtools faidx $GENOMEFOLDER/$REFGENOME
fi
NSTART=$(perl -e "printf('%d',1 + $GVCF_INTERVAL * ($N - 1))")
NEND=$(perl -e "printf('%d',$GVCF_INTERVAL * $N)")
MAX=$(wc -l $GENOMEFOLDER/$REFGENOME.fai | awk '{print $1}')
...
do
	...
	INTERVALS=$(cut -f1 $GENOMEFOLDER/$REFGENOME.fai  | sed -n "${NSTART},${NEND}p" | perl -p -e 's/(\S+)\n/--intervals $1 /g')
	...
	if [ ! -f $GENOVCFOUT.gz ]; then
		...
		time gatk GenotypeGVCFs --reference $GENOMEFOLDER/$REFGENOME --output $GENOVCFOUT -V gendb://$DB --tmp-dir $TEMPDIR
		ls -l $TEMPDIR
		rm -rf $DB
	    fi
	    if [ -f $GENOVCFOUT ]; then
	...
	if [[ ! -f $FILTERSNP.gz || $STEM.$TYPE.vcf.gz -nt $FILTERSNP.gz ]]; then
		...
		--variant $STEM.$TYPE.vcf.gz -R $GENOMEFOLDER/$REFGENOME \
	...
	if [[ ! -f $SELECTSNP.gz || $FILTERSNP.gz -nt $SELECTSNP.gz ]]; then
	    gatk SelectVariants -R $GENOMEFOLDER/$REFGENOME \
